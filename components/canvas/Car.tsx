...
function Porsche({ ...props }) {
  const { scene, nodes, materials } = useGLTF('/gltf/911-transformed.glb')
  useLayoutEffect(() => {
...
const Scene = ({ scale }: { scale: number }) => {
  ...
  return (
    <Canvas shadows camera={{ position: [5, 0, 15], fov: 30 }}>
      ...
      <mesh>
        ...
        <ambientLight intensity={0.5} />
        <Porsche scale={scale} position={[-0.5, -0.18, 0]} rotation={[0, Math.PI / 5, 0]} />
        <AccumulativeShadows position={[0, -1.16, 0]} frames={100} alphaTest={0.9} scale={10}>
          <RandomizedLight amount={8} radius={10} ambient={0.5} position={[1, 5, -1]} />
        </AccumulativeShadows>
        <Environment frames={degraded ? 1 : Infinity} background={false} resolution={256} blur={1}>
          <Lightformers />
        </Environment>
      </mesh>
    </Canvas>
